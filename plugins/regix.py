        ...
        try:
          ...
          async for message in iter_messages(client, chat_id=sts.get("FROM"), limit=sts.get("limit"), offset=sts.get("skip"), filters=filter, max_size=max_size):
                ...
                if forward_tag:
                   ...
                   if ( notcompleted >= 100 
                        or completed <= 100): 
                      await forward(user, client, MSG, m, sts, protect, is_ub)
                      sts.add('total_files', notcompleted)
                      MSG = []
                else:
                   new_caption = custom_caption(message, caption)
                   details = {"msg_id": message.id, "media": media(message), "caption": new_caption, 'button': button, "protect": protect}
                   await copy(user, client, details, m, sts, is_ub)
                   sts.add('total_files')
        except Exception as e:
...
async def restart_pending_forwads(bot, user):
    ...
    try:
       ...
       try: 
          ...
          await msg_edit(m, f"**Source chat may be a private channel / group.**", retry_btn(forward_id), True)
          return await stop(client, user)
       try:
          k = await client.send_message(i.TO, "Testing")
          await k.delete()
       except:
          await msg_edit(m, f"**Please Make Your Bot Admin In Target Channel**", retry_btn(forward_id), True)
          return await stop(client, user)
    except:
       return await db.rmve_frwd(user)
    user_have_db = False
    ...
    if dburi is not None:
        connected, user_db = await connect_user_db(user, dburi, i.TO)
        if not connected:
            await msg_edit(m, "<code>Cannot Connected Your db</code>")
        else:
            user_have_db = True
    try:
...
async def complete_time(total_files, files_per_minute=30):
    minutes_required = total_files / files_per_minute if total_files else 0
